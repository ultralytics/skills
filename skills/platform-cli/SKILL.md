...
| Dataset versions   | `datasets create-export` saves an immutable numbered snapshot and returns its signed NDJSON URL; `datasets export dataset=D` (current data) or `datasets export dataset=D v=N` (a saved version) returns a signed download URL; `datasets restore dataset=D version=N` rolls the live dataset back in place.                                        |
...
| Inference          | `models predict project=P model=M body='{"file":"@image.jpg"}'` or `deployments predict deployment=D body='{"file":"@image.jpg"}'`; `deployments logs`/`metrics` inspect a service. `images predict image_id=ID model_id=ul://...` predicts on a dataset image without saving labels.                                                   |
| Import data        | `datasets create`, then `upload signed-url body=` (`assetType`, `assetId`, `filename`, `contentType`, `totalBytes`), PUT the bytes with the returned headers, `upload complete session_id=S`, `datasets ingest dataset=D body='{"sessionId":"S"}'` (or `{"sourceUrl":...}`), then retrieve until ready. Queued is not imported. |
