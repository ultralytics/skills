...
| Copy               | `datasets clone dataset=D`, `projects clone project=P`, `models clone project=P model=M project_body=DEST`: path `owner`/`project`/`model`/`dataset` name the source; `owner_body`, `project_body`, `model_body`, `dataset_body` name the destination. A model's destination project must already exist.                                                 |
