...
```bash
python .github/scripts/lint_skills.py # the only CI check: skill frontmatter, size limits, openai.yaml, manifest JSON (stdlib only, no install)
...
# Install the local clone as a plugin (Claude Code / Codex); Codex needs a restart after edits
...
- There is no formatter config in this repo (no `pyproject.toml`, no Prettier or codespell config). Formatting is applied by the Ultralytics Actions bot on the PR branch, so expect an `Auto-format by https://ultralytics.com/actions` commit after every push and `git pull --rebase` before pushing again.
- Running `yolo` commands from the repo root is safe for git: `.gitignore` covers `runs/`, `weights/`, `datasets/`, `*.pt`, and every export artifact suffix/directory.

## Workflows

All in `.github/workflows/`:

- `ci.yml` (CI) — one `Lint` job on pushes to `main` and PRs targeting `main`: `actions/checkout`, `actions/setup-python` (3.13), `python .github/scripts/lint_skills.py`. Nothing else runs in CI; it never installs `ultralytics`, so skill content is not executed or fact-checked automatically.
- `format.yml` (Ultralytics Actions) — on PR open/sync/close/review-request and on new issues. Runs Ruff on the lint script (`python: true`), Prettier on YAML/JSON/Markdown (including every `SKILL.md`, companion `.md`, README, and manifest), codespell (`spelling: true`), Lychee broken-link checks (`links: true`), AI labels and PR summaries. It commits fixes back to the PR branch.
- `cla.yml` (CLA) — on `pull_request_target` and issue comments; external contributors sign by commenting `I have read the CLA Document and I sign the CLA` (also `recheck`). Uses `ultralytics/actions/cla@main`.
- `dependabot.yml` — weekly `github-actions` version bumps, labeled `dependencies`.
...
- `SKILL.md` frontmatter is exactly two keys, `name` and `description`, and must be the very first bytes of the file (the linter matches `^---\n...\n---\n` at offset 0). Do not add a license header or blank line above it. `name` equals the directory name, matches `[a-z0-9]+(-[a-z0-9]+)*`, and is ≤64 chars; `description` is ≤1024 chars after whitespace collapsing and is written as a YAML folded scalar (`description: >` followed by indented lines) so it wraps in the file.
...
- The whole `SKILL.md` file, frontmatter included, is capped at 500 newlines by the linter. Version-volatile catalogs (weight names, argument tables, export format matrix, Solutions list) live in flat companion `.md` files next to `SKILL.md` and are referenced from a `## Related pages` section as `` `file.md` (this folder) ``; keep them flat (no subdirectories) and keep their `# Title (v8.4.138)` heading pinned to the grounded version.
...
- `agents/openai.yaml` (Codex/ChatGPT presentation) is `interface:` with exactly two-space-indented, double-quoted, single-line `display_name`, `short_description` (25–64 chars), and `default_prompt` (must contain `$<skill-name>`). It carries the `# Ultralytics 🚀 AGPL-3.0 License - https://ultralytics.com/license` header (as does the lint script); Markdown files carry no header.
...
- `skills/platform-cli/` — the `ul cloud RESOURCE OPERATION key=value` CLI: canonical command shapes, argument and behavior rules (`body=` JSON for union-shaped requests, `@file.json`/`@-` inputs, exit codes 1/2/130, no pagination, owner defaulting), a working method, a goal→commands workflow table, cloud training, recovery (trash/restore), and per-operation gotchas. No companion file; it defers to `ul cloud ... --help`, the [Platform API reference](https://docs.ultralytics.com/platform/api), and `https://platform.ultralytics.com/openapi.json`.
- `.github/scripts/lint_skills.py` — the single validator and the de-facto schema. Per skill dir under `skills/` (any directory counts, so a stray folder fails CI): `SKILL.md` exists, frontmatter regex, exactly `{name, description}` keys, `name == dirname`, name pattern/length, description non-empty and ≤1024, ≤500 newlines, `agents/openai.yaml` exists with `interface:` and the three quoted fields. Then it parses the four manifests as JSON, requires `.claude-plugin/plugin.json` and `.codex-plugin/plugin.json` to agree on `name`, `version`, and `skills`, requires `skills == "./skills/"`, and requires each marketplace to list exactly one plugin whose `name` equals the plugin's. Edit this script to change the format contract; there is no other schema.
...
- **Adding a stage skill** → `skills/<name>/SKILL.md` (frontmatter + body) and `skills/<name>/agents/openai.yaml`; add a row to `## Route before coding` in `skills/yolo/SKILL.md` and to the skill tables in `README.md` and `README.zh-CN.md`; add the name to the `Skill` dropdowns in `.github/ISSUE_TEMPLATE/bug-report.yml` and `feature-request.yml` (both still lack `platform-cli`); update the `description`/`longDescription` and bump `version` in both `plugin.json` files (the linter requires them to match); update the skill count and list in this file; run the lint script.
- **New ultralytics release** → confirm the changed defaults from the installed version, update the `(v8.4.x)` headings and tables in `training-args.md`, `format-matrix.md`, `weights-catalog.md`, any body text that quotes a default, the pinned-version sentence in both READMEs and this file. #11 (8.4.119 → 8.4.138) is the reference diff: 15 files, mostly companion tables.
- **Skill not triggering / triggering too often** → the `description` frontmatter of that skill (trigger keywords, sibling hand-offs) and the router table; nothing else controls activation.
...
- The 500-line cap counts every newline in `SKILL.md`, frontmatter included, and the description cap is measured after collapsing whitespace — a long folded description is fine, but a third frontmatter key (`license`, `metadata`, `allowed-tools`) fails CI.
- `agents/openai.yaml` values must be double-quoted on one line with exactly two spaces of indentation; the linter regex is `^  key: "(.+)"$`, so single quotes, block scalars, or a `$` outside `default_prompt` do not pass. `default_prompt` must literally contain `$<dirname>`.
- Every directory under `skills/` is treated as a skill. Do not park drafts, shared assets, or `__pycache__` there.
- The two `plugin.json` files must agree on `name`, `version`, and `skills` — bump both or CI fails. `.agents/plugins/marketplace.json` and `.claude-plugin/marketplace.json` each must contain exactly one plugin named `yolo`.
...
- Codex picks up skill edits only after a restart; for the GitHub-sourced install run `codex plugin marketplace upgrade ultralytics` then `codex plugin add yolo@ultralytics` to pull a new version (see the README install section).

## Tests

There is no test suite. The only automated check is `python .github/scripts/lint_skills.py` (structure and manifests), run by `ci.yml` on Python 3.13; `format.yml` adds Prettier, Ruff, codespell, and Lychee link checks on PRs. Content correctness is verified manually against the pinned `ultralytics` version and the live Platform CLI/API, and the PR template asks contributors to paste the agent prompt and response they used to verify changed guidance.
