...
- `SKILL.md` frontmatter is exactly two keys, `name` and `description`, and must be the very first bytes of the file. Do not add a license header or blank line above it, and do not add vendor keys (`license`, `metadata`, `allowed-tools`) — the linter rejects any third key. `description` is written as a YAML folded scalar (`description: >` followed by indented lines) so it wraps in the file.
...
There is no test suite. The only automated check is `python .github/scripts/lint_skills.py` (structure and manifests), run by `ci.yml` on Python 3.13; `format.yml` adds Prettier, Ruff, codespell, and Lychee link checks on PRs. Content correctness is verified manually: load this checkout with `claude --plugin-dir .`, run a representative prompt against the edited skill, confirm the agent reads the skill and any companion it needs, and paste the prompt and response into the PR as the template asks. For a `description` change, also try a natural-language prompt that does not name the skill plus a neighboring task that should select its sibling — invoking a skill by name checks the guidance but not the trigger.
